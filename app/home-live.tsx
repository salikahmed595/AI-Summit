...
export function HomeContent() {
  ...
  return <>
    <section className="section upcoming-section" id="upcoming" data-reveal><div className="section-heading"><div><div className="eyebrow">01 / UPCOMING EVENT</div><h2>Be in the room.</h2></div><a className="text-button" href="/events">All events <ArrowUpRight size={18} /></a></div>{featured ? <article className="feature upcoming-card"><div className="event-art">{featured.banner ? <img src={featured.banner} alt={featured.title} /> : <><span>PAICONS PRESENTS</span><h3>{featured.title}</h3></>}</div><div className="feature-copy"><span className="pill">{featured.category || "PAICONS EVENT"}</span><h3>{featured.title}</h3><p className="event-meta">{featured.date}{featured.time ? ` · ${formatTime12(featured.time)}` : ""}<br />{[featured.venue, featured.city].filter(Boolean).join(" · ")}</p><p>{String(cleanDescription(featured.description) || "A focused space to meet people, learn practical ideas and take your next step in AI.").slice(0, 180)}</p><a className="button" href={`/events/${featured.slug}`}>Get Your Pass <ArrowUpRight size={18} /></a></div></article> : <div className="feature empty-upcoming"><div className="event-art"><span>PAICONS</span><h3>New rooms are taking shape.</h3></div><div className="feature-copy"><span className="pill">KARACHI & PAKISTAN</span><h3>Find your next event.</h3><p>Published events will appear here as soon as they are ready.</p><a className="button" href="/events">Explore Events <ArrowUpRight size={18} /></a>{error && <p role="status">Event details are temporarily unavailable.</p>}</div></div>}</section>
    ...
    <section className="orbit-showcase" aria-label="Learn, connect and build"><div className="orbit-art cosmos" aria-hidden="true"><div className="art-grid" /><Cosmos /><span className="art-label">CONNECTED BY CURIOSITY.</span><span className="art-number">24°51′ N<br />67°00′ E</span><div className="art-bottom">LEARN.<br />CONNECT.<br /><b>BUILD.</b></div><MoveUpRight className="big-arrow" size={74} strokeWidth={1} /></div></section>
